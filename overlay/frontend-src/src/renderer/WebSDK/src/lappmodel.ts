...
import { Live2DMotionCaptureFrame } from "./live2d-motion-capture";
...
export class LAppModel extends CubismUserModel {
  public setMotionCaptureFrame(frame: Live2DMotionCaptureFrame | null): void {
    this._motionCaptureFrame = frame;
  }

  private applyMotionCaptureFrame(frame: Live2DMotionCaptureFrame): void {
    const idManager = CubismFramework.getIdManager();

    const add = (parameterId: CubismIdHandle, value: number | undefined, weight = 1.0): void => {
      if (Number.isFinite(value)) this._model.addParameterValueById(parameterId, value, weight);
    };
    const set = (parameterId: CubismIdHandle, value: number | undefined, weight = 1.0): void => {
      if (Number.isFinite(value)) this._model.setParameterValueById(parameterId, value, weight);
    };
    const addNamed = (parameterNames: string[], value: number | undefined, weight = 1.0): void => {
      if (!Number.isFinite(value)) return;
      parameterNames.forEach((name) => this._model.addParameterValueById(idManager.getId(name), value, weight));
    };

    add(this._idParamAngleX, frame.angleX, 0.8);
    add(this._idParamAngleY, frame.angleY, 0.8);
    add(this._idParamAngleZ, frame.angleZ, 0.8);
    add(this._idParamBodyAngleX, frame.bodyAngleX, 0.5);
    add(this._idParamEyeBallX, frame.eyeBallX, 0.8);
    add(this._idParamEyeBallY, frame.eyeBallY, 0.8);
    addNamed(["ParamBodyAngleY", "ParamBodyY"], frame.bodyAngleY, 0.5);
    addNamed(["ParamBodyAngleZ", "ParamBodyZ"], frame.bodyAngleZ, 0.5);
    set(idManager.getId(CubismDefaultParameterId.ParamEyeLOpen), frame.eyeLOpen, 0.7);
    set(idManager.getId(CubismDefaultParameterId.ParamEyeROpen), frame.eyeROpen, 0.7);
    add(idManager.getId(CubismDefaultParameterId.ParamMouthOpenY), frame.mouthOpenY, 0.7);
    add(idManager.getId(CubismDefaultParameterId.ParamMouthForm), frame.mouthForm, 0.7);
    addNamed(["ParamArmLX", "ParamLeftArmX", "ParamArmLeftX", "ParamArmLA"], frame.leftArmX, 0.7);
    addNamed(["ParamArmLY", "ParamLeftArmY", "ParamArmLeftY", "ParamArmLB"], frame.leftArmY, 0.7);
    addNamed(["ParamArmLZ", "ParamLeftArmZ", "ParamArmLeftZ"], frame.leftArmZ, 0.7);
    addNamed(["ParamArmRX", "ParamRightArmX", "ParamArmRightX", "ParamArmRA"], frame.rightArmX, 0.7);
    addNamed(["ParamArmRY", "ParamRightArmY", "ParamArmRightY", "ParamArmRB"], frame.rightArmY, 0.7);
    addNamed(["ParamArmRZ", "ParamRightArmZ", "ParamArmRightZ"], frame.rightArmZ, 0.7);
    addNamed(["ParamForeArmLX", "ParamLeftForeArmX", "ParamLeftElbowX"], frame.leftForearmX, 0.7);
    addNamed(["ParamForeArmLY", "ParamLeftForeArmY", "ParamLeftElbowY"], frame.leftForearmY, 0.7);
    addNamed(["ParamForeArmRX", "ParamRightForeArmX", "ParamRightElbowX"], frame.rightForearmX, 0.7);
    addNamed(["ParamForeArmRY", "ParamRightForeArmY", "ParamRightElbowY"], frame.rightForearmY, 0.7);
    addNamed(["ParamHandLX", "ParamLeftHandX", "ParamWristLX"], frame.leftHandX, 0.7);
    addNamed(["ParamHandLY", "ParamLeftHandY", "ParamWristLY"], frame.leftHandY, 0.7);
    addNamed(["ParamHandRX", "ParamRightHandX", "ParamWristRX"], frame.rightHandX, 0.7);
    addNamed(["ParamHandRY", "ParamRightHandY", "ParamWristRY"], frame.rightHandY, 0.7);
  }
  ...
  public update(): void {
    ...
    if (this._motionCaptureFrame != null) {
      this.applyMotionCaptureFrame(this._motionCaptureFrame);
    }
  ...
  _motionCaptureFrame: Live2DMotionCaptureFrame | null;
